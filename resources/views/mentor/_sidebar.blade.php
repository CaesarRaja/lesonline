...
<aside class="fixed md:hidden left-0 top-0 h-full w-sidebar-width bg-surface-container border-r border-outline-variant flex flex-col p-4 z-40 transition-transform duration-300 ease-in-out -translate-x-full" :class="{ '-translate-x-full': !sidebarOpen, 'translate-x-0': sidebarOpen }">
    ...
    <nav class="flex-1 space-y-1">
        <a @click="sidebarOpen = false" class="flex items-center gap-3 px-4 py-3 rounded-lg font-label-bold text-label-bold transition-colors {{ $active === 'dashboard' ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-variant' }}" href="{{ route('mentor.dashboard') }}">
            <span class="material-symbols-outlined {{ $active === 'dashboard' ? 'fill-icon' : '' }}">dashboard</span> Dashboard
        </a>
        <a @click="sidebarOpen = false" class="flex items-center gap-3 px-4 py-3 rounded-lg font-label-bold text-label-bold transition-colors {{ $active === 'schedules' ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-variant' }}" href="{{ route('mentor.schedules') }}">
            <span class="material-symbols-outlined {{ $active === 'schedules' ? 'fill-icon' : '' }}">calendar_month</span> Jadwal
        </a>
        <a @click="sidebarOpen = false" class="flex items-center gap-3 px-4 py-3 rounded-lg font-label-bold text-label-bold transition-colors {{ $active === 'materials' ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-variant' }}" href="{{ route('mentor.materials') }}">
            <span class="material-symbols-outlined {{ $active === 'materials' ? 'fill-icon' : '' }}">cloud_upload</span> Upload Materi
        </a>
        <a @click="sidebarOpen = false" class="flex items-center gap-3 px-4 py-3 rounded-lg font-label-bold text-label-bold transition-colors {{ $active === 'withdrawals' ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-variant' }}" href="{{ route('mentor.withdrawals') }}">
            <span class="material-symbols-outlined {{ $active === 'withdrawals' ? 'fill-icon' : '' }}">account_balance</span> Penarikan Saldo
        </a>
        <a @click="sidebarOpen = false" class="flex items-center gap-3 px-4 py-3 rounded-lg font-label-bold text-label-bold transition-colors {{ $active === 'settings' ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-variant' }}" href="{{ route('mentor.settings') }}">
            <span class="material-symbols-outlined {{ $active === 'settings' ? 'fill-icon' : '' }}">badge</span> Detail Mentor
        </a>
        <a @click="sidebarOpen = false" class="flex items-center gap-3 px-4 py-3 rounded-lg font-label-bold text-label-bold transition-colors text-on-surface-variant hover:bg-surface-variant" href="{{ route('mentor.export-pdf') }}">
            <span class="material-symbols-outlined">picture_as_pdf</span> Export Laporan
        </a>
    </nav>
